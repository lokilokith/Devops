...
def validate_access_request_create(data: dict):
    ...
    if len(data["business_justification"]) < 10:
        raise UnprocessableEntity("business_justification must be at least 10 characters long")
    ...
    if not has_role and not has_resource:
        raise UnprocessableEntity("Must specify either requested_role_id or requested_resource_id.")
        
    if has_role and has_resource:
        raise UnprocessableEntity("Cannot specify both requested_role_id and requested_resource_id.")
    ...
    if "requested_start" in data and data["requested_start"] and "requested_end" in data and data["requested_end"]:
        try:
            start = datetime.fromisoformat(data["requested_start"].replace('Z', '+00:00'))
            end = datetime.fromisoformat(data["requested_end"].replace('Z', '+00:00'))
            if start >= end:
                raise UnprocessableEntity("requested_start must be before requested_end.")
        except ValueError:
