...
approval_workflows_ns = Namespace(
    "approval-workflows", description="Approval Workflows operations"
)
...
approval_workflow_response_model = approval_workflows_ns.model(
    "ApprovalWorkflowResponse",
    {
        "id": fields.String(description="UUID of workflow"),
        "access_request_id": fields.String(
            description="UUID of associated access request"
        ),
        ...
        "requested_role_name": fields.String(
            attribute=lambda x: (
                x.access_request.role.role_name
                if x.access_request and x.access_request.role
                else None
            ),
        ...
        "requested_resource_name": fields.String(
            attribute=lambda x: (
                x.access_request.resource.resource_name
                if x.access_request and x.access_request.resource
                else None
            ),
...
approval_workflow_list_response_model = approval_workflows_ns.model(
...
approval_workflow_single_response_model = approval_workflows_ns.model(
    "ApprovalWorkflowSingleResponse",
    {
        "success": fields.Boolean(default=True),
        "message": fields.String(default="Success"),
...
approval_action_model = approval_workflows_ns.model(
    "ApprovalAction",
    {
        "comments": fields.String(
            description="Optional justification/comment", required=False
        )
    },
)
...
approval_workflow_query_parser.add_argument(
    "page", type=int, required=False, default=1, help="Page number"
)
approval_workflow_query_parser.add_argument(
    "page_size", type=int, required=False, default=50, help="Items per page"
)
approval_workflow_query_parser.add_argument(
    "status",
    type=str,
...
approval_workflow_query_parser.add_argument(
    "request_id", type=str, required=False, help="Filter by access request ID"
)
approval_workflow_query_parser.add_argument(
    "approver_id", type=str, required=False, help="Filter by approver ID"
)
approval_workflow_query_parser.add_argument(
    "created_after", type=str, required=False, help="Filter by creation time (ISO 8601)"
)
approval_workflow_query_parser.add_argument(
    "created_before",
    type=str,
    required=False,
    help="Filter by creation time (ISO 8601)",
)
