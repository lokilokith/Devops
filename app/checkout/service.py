...
class CheckoutService:
        ...
        try:
            with self._session.begin_nested():
                ...
                if ar.requester_id != user_id:
                    raise UnauthorizedCheckoutError(
                        "Request does not belong to the user"
                    )

                # 3. Verify request is APPROVED
                if ar.status != AccessRequestStatus.APPROVED:
                    raise InvalidCheckoutStateError(
                        f"Request is not APPROVED (status: {ar.status.value})"
                    )
                ...
                if ar.requested_end:
                    ...
                    if ar_end < now:
                        raise InvalidCheckoutStateError(
                            "Access request window has expired"
                        )

                # 5. Authorize User against Baseline RBAC for Vault Retrieval
                from app.authorization.exceptions import AuthorizationDeniedError
                from app.permissions.models import PermissionAction

                try:
                    self._authz.authorize(user_id, "vault", PermissionAction("read"))
                except AuthorizationDeniedError as err:
                    self._audit.log_event(
                        actor_user_id=user_id,
                        action="CHECKOUT_DENIED_RBAC",
                    ...
                    raise UnauthorizedCheckoutError(
                        "User lacks RBAC permission to checkout Vault secrets."
                    ) from err
                ...
                if not secret:
                    raise CheckoutError(
                        "No VaultSecret found for the requested resource"
                    )

                # 7. Verify VaultSecret is ACTIVE
                if secret.status != SecretStatus.ACTIVE:
                    raise InvalidCheckoutStateError(
                        f"VaultSecret is not ACTIVE (status: {secret.status.value})"
                    )

                # 8. Evaluate Dynamic Policy rules via PolicyEngine
                try:
                    ...
                    self._audit.log_event(
                        actor_user_id=user_id,
                        action="CHECKOUT_DENIED_POLICY_ERROR",
                        resource_type="vault_secrets",
                        resource_id=str(secret.id),
                        status=AuditStatus.FAILED,
                        severity=AuditSeverity.HIGH,
                        details={"reason": "Policy engine exception", "error": str(e)},
                    )
                    raise CheckoutError("Policy evaluation failed") from e

                if decision == PolicyDecision.DENY:
                    self._audit.log_event(
                        actor_user_id=user_id,
                        action="CHECKOUT_DENIED_POLICY",
                        resource_type="vault_secrets",
                        resource_id=str(secret.id),
                        status=AuditStatus.DENIED,
                        severity=AuditSeverity.HIGH,
                    ...
                    raise PolicyDeniedError("Checkout explicitly denied by policy.")
                ...
                if decision == PolicyDecision.REQUIRE_APPROVAL:
                    self._audit.log_event(
                        ...
                        severity=AuditSeverity.HIGH,
                        details={
                            "reason": "Policy REQUIRE_APPROVAL not satisfied by provided request",
                            "access_request_id": str(access_request_id),
                        },
                    )
                    raise PolicyDeniedError(
                        "Policy requires approval but the provided request did not satisfy the policy engine."
                    )

                # 9. Atomically transition ACTIVE -> CHECKED_OUT using row_version CAS
                secret.checkout()
            ...
            self._session.commit()
            raise InvalidCheckoutStateError(
                "Checkout conflict: another user checked out this secret."
            )
        except Exception as e:
            self._session.rollback()
            self._audit.log_event(
                ...
                resource_type="vault_secrets",
                resource_id=(
                    str(ar.requested_resource_id) if "ar" in locals() and ar else None
                ),
                status=AuditStatus.DENIED,
                severity=AuditSeverity.HIGH,
    ...
    def checkin(self, user_id: UUID, lease_id: UUID) -> None:
        ...
        if lease.status != LeaseStatus.ACTIVE:
            raise InvalidCheckoutStateError(
                f"Lease is not ACTIVE (status: {lease.status.value})"
            )
    ...
    def expire(self, lease_id: UUID) -> None:
        ...
        self._checkin_internal(
            lease, system_actor_id, LeaseStatus.EXPIRED, "LEASE_EXPIRED"
        )
    ...
    def revoke(self, admin_id: UUID, lease_id: UUID) -> None:
        ...
        try:
            ...
            raise UnauthorizedCheckoutError(
                "Admin is not authorized to revoke leases"
            ) from e
