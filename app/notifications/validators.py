...
from app.notifications.models import NotificationStatus, NotificationType, NotificationPriority
...
def validate_filter_params(args: dict) -> dict:
    ...
    if args.get("status"):
        try:
            ...
            raise BadRequest(f"Invalid status. Must be one of {[e.value for e in NotificationStatus]}")
            
    if args.get("type"):
        try:
            parsed["type_"] = NotificationType(args["type"])
        except ValueError:
            raise BadRequest(f"Invalid type. Must be one of {[e.value for e in NotificationType]}")
            
    if args.get("priority"):
        try:
            parsed["priority"] = NotificationPriority(args["priority"])
        except ValueError:
            raise BadRequest(f"Invalid priority. Must be one of {[e.value for e in NotificationPriority]}")
    ...
    if args.get("created_after"):
        try:
            parsed["created_after"] = datetime.fromisoformat(args["created_after"].replace("Z", "+00:00"))
        except ValueError:
            raise BadRequest("Invalid created_after date format (ISO 8601 expected).")
            
    if args.get("created_before"):
        try:
            parsed["created_before"] = datetime.fromisoformat(args["created_before"].replace("Z", "+00:00"))
        except ValueError:
