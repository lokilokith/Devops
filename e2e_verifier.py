...
def run_users_workflow(token):
    ...
    resp = requests.get(f"{BASE_URL}/users", headers=headers, params={"search": username})
    if resp.json()["data"]:
    ...
    resp = requests.put(f"{BASE_URL}/users/{user_id}", headers=headers, json=edit_payload)
    ...
    resp = requests.put(f"{BASE_URL}/users/{user_id}", headers=headers, json=disable_payload)
    ...
    resp = requests.put(f"{BASE_URL}/users/{user_id}", headers=headers, json=enable_payload)
    ...
    requests.post(f"{BASE_URL}/users", headers=headers, json=payload) # leave one active
    resp = requests.get(f"{BASE_URL}/users", headers=headers, params={"search": "E2E", "limit": 1, "skip": 0})
...
def run_access_request_workflow(admin_token):
    ...
    resp = requests.get(f"{BASE_URL}/users", headers=headers, params={"search": "requester"})
    if resp.json()["data"]:
        for u in resp.json()["data"]:
            requests.delete(f"{BASE_URL}/users/{u['id']}", headers=headers)
    ...
    resp = requests.post(f"{BASE_URL}/access-requests", headers=req_headers, json=ar_payload)
    ...
    resp = requests.post(f"{BASE_URL}/access-requests/{ar_id1}/cancel", headers=req_headers)
    ...
    resp = requests.get(f"{BASE_URL}/access-requests", headers=req_headers, params={"search": "e2e", "status": "pending"})
    assert resp.status_code == 200
    assert len(resp.json()["data"]) >= 1
    ...
    time.sleep(1)
    aw_resp = requests.get(f"{BASE_URL}/approval-workflows", headers=headers, params={"status": "pending"})
    aw = None
    ...
    resp = requests.post(f"{BASE_URL}/approval-workflows/{aw['id']}/approve", headers=headers, json={"comments": "Approved for e2e"})
    ...
    resp = requests.post(f"{BASE_URL}/approval-workflows/{aw['id']}/reject", headers=headers, json={"comments": "Rejected for e2e"})
    assert resp.status_code == 200
...
def run_db_validation():
    ...
    assert len(active_users) <= 1, f"Found multiple active users with same username: {active_users}"
    print("[SUCCESS] DB: No duplicate active users")
    
    print("Checking access_requests status...")
    cur.execute("SELECT status FROM access_requests WHERE business_justification LIKE '%e2e%'")
