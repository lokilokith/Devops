...
def test_approve_reject_cancel(ar_repo, sample_user, sample_role):
    req1 = ar_repo.create_request(AccessRequest(
        request_number="REQ-A1", requester_id=sample_user.id, requested_role_id=sample_role.id, business_justification="Test"
    ))
    ...
    req2 = ar_repo.create_request(AccessRequest(
        request_number="REQ-A2", requester_id=sample_user.id, requested_role_id=sample_role.id, business_justification="Test"
    ))
    rejected = ar_repo.reject(req2.id, "No.")
    assert rejected.status == AccessRequestStatus.REJECTED
    assert rejected.rejected_reason == "No."

    req3 = ar_repo.create_request(AccessRequest(
        request_number="REQ-A3", requester_id=sample_user.id, requested_role_id=sample_role.id, business_justification="Test"
    ))
...
def test_list_and_count_and_search(ar_repo, sample_user, sample_role, sample_resource):
    ar_repo.create_request(AccessRequest(request_number="REQ-L1", requester_id=sample_user.id, requested_role_id=sample_role.id, business_justification="Test"))
    ar_repo.create_request(AccessRequest(request_number="REQ-L2", requester_id=sample_user.id, requested_resource_id=sample_resource.id, business_justification="Test", status=AccessRequestStatus.APPROVED))
...
def test_sqlalchemy_errors(ar_repo, sample_user, sample_role):
    ...
    with patch.object(ar_repo._session, 'add', side_effect=SQLAlchemyError("mocked error")):
        with pytest.raises(AccessRequestRepositoryError):
            ar_repo.create_request(req)
            
    with patch.object(ar_repo._session, 'execute', side_effect=SQLAlchemyError("mocked error")):
        with pytest.raises(AccessRequestRepositoryError):
    ...
    req2 = ar_repo.create_request(AccessRequest(
        request_number="REQ-ERR2", requester_id=sample_user.id, requested_role_id=sample_role.id, business_justification="Test"
    ))
    
    with patch.object(ar_repo._session, 'commit', side_effect=SQLAlchemyError("mocked error")):
        with pytest.raises(AccessRequestRepositoryError):
