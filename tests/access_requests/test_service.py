...
def test_submit_request_validations(ar_service, sample_user, sample_role):
    ...
    with pytest.raises(
        AccessRequestValidationError, match="Must specify either a role or a resource"
    ):
    ...
    with pytest.raises(
        AccessRequestValidationError, match="start must be before requested end"
    ):
    ...
    # Unknown resource
    with pytest.raises(
        AccessRequestValidationError, match="Requested resource not found"
    ):
...
def test_duplicate_request(ar_service, sample_user, sample_role):
    ...
    with pytest.raises(AccessRequestDuplicateError):
        ar_service.submit_request(
            sample_user.id, "Test again", requested_role_id=sample_role.id
        )
