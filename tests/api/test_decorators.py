...
def create_mock_token(app, user_id, expired=False, revoked=False, invalid=False, token_type="access"):
    if invalid:
    ...
    payload = {
        "sub": str(user_id),
        "type": token_type,
        "exp": datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(hours=-1 if expired else 1),
...
def test_requires_permission_success(app, client, db_session):
    setup_app_routes(app)
    u = User(employee_id="P1", username="p1", email="p1@a.c", full_name="P1", status=UserStatus.ACTIVE)
    r = Role(role_code="ADM", role_name="Admin", role_type=RoleType.SYSTEM, status=RoleStatus.ACTIVE)
    res_obj = Resource(resource_code="ADMIN_RES", resource_name="Admin Res")
    perm = Permission(permission_code="PERM_ADMIN_RES_READ", permission_name="Admin Res", action=PermissionAction.READ, description="read admin", status="active")
...
def test_requires_permission_denied(app, client, db_session):
    setup_app_routes(app)
    u = User(employee_id="P2", username="p2", email="p2@a.c", full_name="P2", status=UserStatus.ACTIVE)
    db_session.add(u)
    db_session.commit()
