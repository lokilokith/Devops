...
def test_create_initial_workflow(approval_service, sample_users, sample_request):
    ...
    with pytest.raises(ApprovalWorkflowValidationError, match="Access request not found"):
        approval_service.create_initial_workflow(uuid4(), u2.id)

def test_approve_workflow(approval_service, approval_repo, ar_repo, ur_repo, sample_users, sample_request, sample_role):
    u1, u2 = sample_users
    wf = approval_service.create_initial_workflow(sample_request.id, u2.id)
    ...
    with pytest.raises(UserRoleAlreadyExistsError):
        ur_repo.assign_role_to_user(user_id=u1.id, role_id=sample_role.id, assigned_by_user_id=u2.id)
...
def test_approve_duplicate_assignment_silent(approval_service, ur_repo, sample_users, sample_request, sample_role):
    u1, u2 = sample_users
    wf = approval_service.create_initial_workflow(sample_request.id, u2.id)
    ur_repo.assign_role_to_user(user_id=u1.id, role_id=sample_role.id, assigned_by_user_id=u2.id)
...
def test_cancel_workflow_for_request(approval_service, approval_repo, sample_users, sample_request):
    u1, u2 = sample_users
