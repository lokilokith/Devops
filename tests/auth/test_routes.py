...
def test_login_success(client, sample_user):
    ...
    res = client.post("/auth/login", json={"username": sample_user.username, "password": "password"})
...
def test_login_wrong_password(client, sample_user):
    ...
    res = client.post("/auth/login", json={"username": sample_user.username, "password": "wrong"})
    assert res.status_code == 401

def test_login_unknown_user(client):
    res = client.post("/auth/login", json={"username": "unknown", "password": "password"})
    assert res.status_code == 401
