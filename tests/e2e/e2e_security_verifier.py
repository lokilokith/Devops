...
def main():
    ...
    assert r.status_code in [
        ...
        422,
        500,
    ], f"Invalid UUID returned {r.status_code}"
    ...
    r = requests.post(
        f"{BASE_URL}/vault/secrets",
        headers=admin_headers,
        json={"resource_id": res_id, "payload": payload_plaintext},
    )
    ...
    cur = conn.cursor()
    cur.execute(
        f"SELECT encrypted_payload::text FROM vault_secret_versions WHERE secret_id = '{secret_id}'"
    )
    out = cur.fetchone()[0]
    ...
    r = requests.post(
        ...
        headers=admin_headers,
        json={"payload": "new_rotated_value", "resource_id": res_id},
    )
