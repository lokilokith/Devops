...
def verify_audit_log(
    headers, resource_id, expected_action, actor_id=None, resource_type="vault_secrets"
):
    ...
    print(
        f"DEBUG Audit Logs length: {len(logs)}. Searching for resource: {resource_id} action: {expected_action}"
    )
    for log in logs:
        if log["resource_id"] == resource_id and log["action"] == expected_action:
            if actor_id and log.get("user_id") != actor_id:
                ...
                out = subprocess.check_output(
                    [
                        ...
                        "opsforge",
                        "-c",
                        f"SELECT id, action, actor_user_id FROM audit_logs WHERE resource_id = '{resource_id}'",
                    ],
                    text=True,
                )
            ...
            return True
    print(
        f"DEBUG: missing audit log. Found logs: {[(log['resource_id'], log['action']) for log in logs]}"
    )
    return False
...
def verify_api_contract_stats(resp_data):
    keys = resp_data.keys()
    if (
        "total_secrets" not in keys
        or "active_secrets" not in keys
        or "disabled_secrets" not in keys
    ):
        return False
    return True
...
def main():
    ...
    _admin_id = r.json()["data"]["id"]  # noqa: F841
    ...
    r = requests.post(
        ...
        headers=admin_headers,
        json={"resource_id": res_id, "payload": payload},
    )
    # print("Secret creation response:", r.json())
    assert r.status_code == 201, f"Failed to create secret: {r.text}"
    secret_id = r.json()["data"]["id"]
    assert verify_api_contract_list(
        [r.json()["data"]]
    ), "API Contract violation on CREATE (leaked payload)"

    assert verify_audit_log(
        admin_headers, secret_id, "SECRET_CREATED"
    ), "Audit log SECRET_CREATED missing"
    ...
    assert (
        r.status_code == 201
    ), f"Failed to create access request: {r.status_code} {r.text}"
    req_id = r.json()["data"]["id"]

    # Approve Access Request
    print_step("Approve Access Request")
    r = requests.post(
        f"{BASE_URL}/access-requests/{req_id}/approve", headers=admin_headers
    )
    assert (
        r.status_code == 200
    ), f"Failed to approve access request: {r.status_code} {r.text}"

    # Retrieve Secret
    r = requests.post(
        f"{BASE_URL}/vault/secrets/{secret_id}/retrieve", headers=admin_headers
    )
    ...
    assert verify_audit_log(
        admin_headers, secret_id, "SECRET_RETRIEVED"
    ), "Audit log SECRET_RETRIEVED missing"

    # Rotate Secret
    new_payload = "rotated_secret"
    r = requests.post(
        f"{BASE_URL}/vault/secrets/{secret_id}/rotate",
        headers=admin_headers,
        json={"payload": new_payload, "resource_id": res_id},
    )
    assert r.status_code == 200, f"Failed to rotate secret: {r.status_code} {r.text}"

    assert verify_audit_log(
        admin_headers, secret_id, "SECRET_ROTATED"
    ), "Audit log SECRET_ROTATED missing"
    ...
    # Disable Secret
    r = requests.post(
        f"{BASE_URL}/vault/secrets/{secret_id}/disable", headers=admin_headers
    )
    assert r.status_code == 200, "Failed to disable secret"

    assert verify_audit_log(
        admin_headers, secret_id, "SECRET_DISABLED"
    ), "Audit log SECRET_DISABLED missing"

    print_step("Vault Stats Contract")
    r = requests.get(f"{BASE_URL}/vault/secrets/stats", headers=admin_headers)
    assert r.status_code == 200
    assert verify_api_contract_stats(
        r.json()["data"]
    ), "API Contract violation on STATS"
    ...
    soc_role_id = next(
        (role["id"] for role in r.json()["data"] if "SOC" in role["role_name"]), None
    )
    if not soc_role_id:
        r = requests.post(
            f"{BASE_URL}/roles",
            ...
            json={
                ...
                "role_code": f"SOC_{uuid.uuid4().hex[:4].upper()}",
            },
        )
        soc_role_id = r.json()["data"]["id"]

    r = requests.post(
        f"{BASE_URL}/users/{soc_user_id}/roles",
        headers=admin_headers,
        json={"role_id": soc_role_id},
    )
    ...
    r = requests.get(
        f"{BASE_URL}/permissions?search=PERM_VAULT_READ", headers=admin_headers
    )
    perms = [
        p for p in r.json().get("data", []) if p["permission_code"] == "PERM_VAULT_READ"
    ]
    ...
    r = requests.post(
        f"{BASE_URL}/roles/{soc_role_id}/permissions",
        headers=admin_headers,
    ...
    r = requests.patch(
        f"{BASE_URL}/permissions/{perm_id}",
        headers=admin_headers,
        json={"status": "inactive"},
    )
    assert (
        r.status_code == 200
    ), f"Failed to patch permission to inactive: {r.status_code} {r.text}"

    # Create a new secret for SOC test since the previous one was disabled
    r = requests.post(
        f"{BASE_URL}/vault/secrets",
        headers=admin_headers,
        json={"resource_id": res_id, "payload": "soc_test_payload"},
    )
    ...
    r = requests.post(
        f"{BASE_URL}/vault/secrets/{soc_secret_id}/retrieve", headers=soc_headers
    )
    print("SOC Retrieve Status (with INACTIVE perm):", r.status_code)
    assert (
        r.status_code == 403
    ), f"Expected 403 Forbidden because permission is INACTIVE, got {r.status_code}"
    ...
    r = requests.post(
        f"{BASE_URL}/vault/secrets",
        headers=admin_headers,
        json={"resource_id": res_id_2, "payload": "secret_2_payload"},
    )
    ...
    r = requests.post(
        f"{BASE_URL}/vault/secrets/{secret_id_2}/retrieve", headers=soc_headers
    )
    assert r.status_code == 403, f"Expected 403 APPROVAL_REQUIRED, got {r.status_code}"
    assert (
        r.json().get("error") == "APPROVAL_REQUIRED"
    ), f"Expected APPROVAL_REQUIRED error, got {r.json()}"
    ...
    # Admin Approves
    r = requests.post(
        f"{BASE_URL}/access-requests/{soc_req_id}/approve", headers=admin_headers
    )
    ...
    assert (
        r.status_code == 200
    ), f"SOC Analyst should retrieve secret successfully after approval, got {r.status_code} {r.text}"
    assert (
        r.json()["data"]["payload"] == "secret_2_payload"
