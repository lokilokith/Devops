...
def main():
    ...
    for i in range(N):
        ...
        r = requests.post(
            ...
            headers=headers,
            json={"resource_id": res_id, "payload": f"perf_payload_{i}"},
        )
    ...
    for res_id, sec_id in secrets:
        req = requests.post(
            ...
            json={
                ...
                "business_justification": "Perf test retrieving secrets",
                "duration_hours": 1,
            },
        )
        ...
        r = requests.post(
            f"{BASE_URL}/vault/secrets/{sec_id}/retrieve",
            headers=headers,
            json={"reason": "perf retrieve"},
        )
        assert r.status_code == 200, f"Retrieve failed: {r.status_code}"
    print(
        f"Retrieved {N} secrets (with Access Requests) in {time.time() - start_time:.2f}s"
    )
    ...
    for i, (res_id, sec_id) in enumerate(secrets):
        r = requests.post(
            f"{BASE_URL}/vault/secrets/{sec_id}/rotate",
            headers=headers,
            json={"resource_id": res_id, "payload": f"perf_rotated_{i}"},
        )
    ...
    for res_id, sec_id in secrets:
        r = requests.post(
            f"{BASE_URL}/vault/secrets/{sec_id}/disable",
            headers=headers,
