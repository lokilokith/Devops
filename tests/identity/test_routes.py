...
def test_list_users(client, mock_auth, mock_service):
    ...
    res = client.get("/users?skip=0&limit=10", headers={"Authorization": "Bearer token"})
...
def test_update_user(client, mock_auth, mock_service):
    ...
    res = client.put(f"/users/{uid}", json={"full_name": "Updated", "status": "active"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 200
...
def test_patch_user(client, mock_auth, mock_service):
    ...
    res = client.patch(f"/users/{uid}", json={"full_name": "Patched"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 200
...
def test_validation_errors(client, mock_auth, mock_service):
    ...
    res = client.post("/users", json={
        "employee_id": "E1", "username": "testuser", "email": "bad", "full_name": "Test User", "password": "password123"
    }, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422
    
    # Username too short
    res = client.post("/users", json={
        "employee_id": "E1", "username": "ab", "email": "test@test.com", "full_name": "Test User", "password": "password123"
    }, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422
    ...
    # Password too short
    res = client.post("/users", json={
        "employee_id": "E1", "username": "testuser", "email": "test@test.com", "full_name": "Test User", "password": "short"
    }, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422
    
    # Missing employee id value
    res = client.post("/users", json={
        "employee_id": "", "username": "testuser", "email": "test@test.com", "full_name": "Test User", "password": "password123"
    }, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422
    
    # Invalid manager id
    res = client.post("/users", json={
        "employee_id": "E1", "username": "testuser", "email": "test@test.com", "full_name": "Test User", "password": "password123", "manager_user_id": "bad-uuid"
    }, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422
    
    # Invalid update missing full_name
    uid = str(uuid4())
    res = client.put(f"/users/{uid}", json={"full_name": "", "status": "active"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422
    
    # Invalid update missing status
    res = client.put(f"/users/{uid}", json={"full_name": "Updated", "status": ""}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422
