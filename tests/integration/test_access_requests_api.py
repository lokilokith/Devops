...
def auth_setup(db_session, app):
    ...
    for p in perms:
        perm = Permission(permission_code=f"PERM_ACCESS_REQUESTS_{p.upper()}", permission_name=f"access_requests.{p}", action=PermissionAction(p))
        db_session.add(perm)
        db_session.flush()
...
def test_approve_access_request_forbidden(client, db_session):
    ...
    user = User(employee_id="TEST_002", username="testuser2", email="test2@local", full_name="Test User 2", status=UserStatus.ACTIVE)
    auth_svc = AuthService(IdentityRepository(db_session))
    user.password_hash = auth_svc.hash_password("secret")
    db_session.add(user)
    db_session.commit()
