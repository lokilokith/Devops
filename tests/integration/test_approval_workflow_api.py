...
def test_user(db_session):
    ...
    user = User(username="test_user", email="test@user.com", full_name="Test User", employee_id="T01")
    db_session.add(user)
    db_session.commit()
    return user
...
def auth_headers(test_user, db_session):
    ...
    ar_perm = Permission(permission_code="PERM_ACCESS_REQUESTS_READ", permission_name="access_requests.read", action=PermissionAction("read"))
...
def test_get_approval_workflow_by_id(client, auth_headers, test_workflow):
    response = client.get(f"/approval-workflows/{test_workflow.id}", headers=auth_headers)
    assert response.status_code == 200
    assert response.json["success"] is True
    assert response.json["data"]["id"] == str(test_workflow.id)

def test_approve_workflow(client, auth_headers, test_workflow):
    response = client.post(f"/approval-workflows/{test_workflow.id}/approve", json={"comments": "LGTM"}, headers=auth_headers)
    assert response.status_code == 200
    assert response.json["success"] is True
...
def test_reject_workflow_after_approve_fails(client, auth_headers, test_workflow):
    # First approve
    client.post(f"/approval-workflows/{test_workflow.id}/approve", json={"comments": "LGTM"}, headers=auth_headers)
    
    # Try to reject
    response = client.post(f"/approval-workflows/{test_workflow.id}/reject", json={"comments": "Wait no"}, headers=auth_headers)
...
def test_get_workflows_for_request(client, auth_headers, test_workflow):
    response = client.get(f"/access-requests/{test_workflow.access_request_id}/approval-workflow", headers=auth_headers)
    assert response.status_code == 200
    assert response.json["success"] is True
    assert len(response.json["data"]) > 0
