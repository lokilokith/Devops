...
def test_e2e_approval_workflow_full_lifecycle(client, db_session):
    ...
    admin_user = User(username="e2e_admin", email="admin@e2e.com", full_name="Admin E2E", employee_id="A01")
    db_session.add(admin_user)
    
    # Create standard user
    standard_user = User(username="e2e_user", email="user@e2e.com", full_name="Standard E2E", employee_id="S01")
    ...
    admin_role = Role(role_code="E2E_ADMIN_ROLE", role_name="E2E Admin Role", role_type=RoleType.CUSTOM)
    user_role = Role(role_code="E2E_USER_ROLE", role_name="E2E User Role", role_type=RoleType.CUSTOM)
    ...
    for p in perms:
        perm = Permission(permission_code=f"PERM_APPROVAL_WORKFLOWS_{p.upper()}", permission_name=f"approval_workflows.{p}", action=PermissionAction(p))
        db_session.add(perm)
        db_session.flush()
        db_session.add(RolePermission(role_id=admin_role.id, permission_id=perm.id))
        
    ar_read_perm = Permission(permission_code="PERM_ACCESS_REQUESTS_READ", permission_name="access_requests.read", action=PermissionAction("read"))
    ar_create_perm = Permission(permission_code="PERM_ACCESS_REQUESTS_CREATE", permission_name="access_requests.create", action=PermissionAction("create"))
    ...
    db_session.add(RolePermission(role_id=user_role.id, permission_id=ar_create_perm.id))
    ...
    response = client.post("/access-requests", json={
    ...
    response = client.post(f"/approval-workflows/{wf.id}/approve", json={"comments": "Approved!"}, headers=admin_headers)
    assert response.status_code == 200
    assert response.json["data"]["status"] == "approved"
    
    # 6. Verify role provisioning via API or DB
    ur = db_session.query(UserRole).filter_by(user_id=standard_user.id, role_id=target_role.id).first()
