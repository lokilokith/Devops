...
def test_e2e_approval_workflow_full_lifecycle(client, db_session):
    ...
    target_role = Role(
        role_code=f"R_{uuid4().hex[:8]}", role_name=f"Role {uuid4().hex[:8]}"
    )
    ...
    ar_read_perm = db_session.scalar(
        select(Permission).where(
    ...
    if not ar_read_perm:
        ...
        db_session.flush()
    ar_create_perm = db_session.scalar(
        select(Permission).where(
            Permission.permission_code == "PERM_ACCESS_REQUESTS_CREATE"
        )
    )
