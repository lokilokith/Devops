...
def test_access_request_creates_workflow(client, user_token, db_session, test_role):
    ...
    workflow = (
        db_session.query(ApprovalWorkflow)
        .filter_by(access_request_id=UUID(req_id))
        .first()
    )
    assert workflow is not None


def test_pending_request_visible_in_approval_queue(
    client, admin_token, user_token, test_role
):
    res = client.post(
        "/access-requests",
        json={
    ...
    queue_res = client.get(
        "/approval-workflows", headers={"Authorization": f"Bearer {admin_token}"}
    )
...
def test_approval_assigns_permission(
    client, admin_token, user_token, test_role, normal_user, db_session
):
    ...
    ur = (
        ...
        .filter_by(user_id=normal_user.id, role_id=test_role.id)
        .first()
    )
    assert ur is not None


def test_request_rejected_no_permission(
    client, admin_token, user_token, test_role, normal_user, db_session
):
    from app.roles.models import UserRole
