...
def setup_admin_user(db_session, app):
    ...
    u = User(employee_id="ADM1", username="admin_int", email="admin_int@test.com", full_name="Admin", status=UserStatus.ACTIVE)
    db_session.add(u)
    
    r = Role(role_code="ADMIN_INT", role_name="Admin Integration", role_type=RoleType.SYSTEM, status=RoleStatus.ACTIVE)
    db_session.add(r)
    
    # We need permissions for "permissions" resource
    p1 = Permission(permission_code="PERM_PERMISSIONS_READ", permission_name="permissions.read", action=PermissionAction.READ)
    p2 = Permission(permission_code="PERM_PERMISSIONS_CREATE", permission_name="permissions.create", action=PermissionAction.CREATE)
    p3 = Permission(permission_code="PERM_PERMISSIONS_UPDATE", permission_name="permissions.update", action=PermissionAction.UPDATE)
    p4 = Permission(permission_code="PERM_PERMISSIONS_DELETE", permission_name="permissions.delete", action=PermissionAction.DELETE)
...
def setup_limited_user(db_session, app):
    u = User(employee_id="LIM1", username="limited_int", email="lim_int@test.com", full_name="Limited", status=UserStatus.ACTIVE)
    db_session.add(u)
    db_session.commit()
    from tests.api.test_decorators import create_mock_token
    token = create_mock_token(app, u.id)
