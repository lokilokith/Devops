...
def test_role_permissions_crud(client, db_session, app):
    ...
    assign_data = {"permission_id": str(perm.id)}
    res = client.post(
        f"/roles/{admin_role_id}/permissions", json=assign_data, headers=headers
    )
    ...
    db_rp = (
        ...
        .filter_by(role_id=admin_role_id, permission_id=perm.id)
        .first()
    )
    ...
    res_del = client.delete(
        f"/roles/{admin_role_id}/permissions/{perm.id}", headers=headers
    )
