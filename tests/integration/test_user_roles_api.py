...
def setup_admin_user(db_session, app):
    u = User(employee_id="ADM3", username="admin_ur_int", email="admin_ur@test.com", full_name="Admin", status=UserStatus.ACTIVE)
    db_session.add(u)
...
def setup_limited_user(db_session, app):
    u = User(employee_id="LIM3", username="limited_ur_int", email="lim_ur_int@test.com", full_name="Limited", status=UserStatus.ACTIVE)
    db_session.add(u)
    db_session.commit()
    from tests.api.test_decorators import create_mock_token
    token = create_mock_token(app, u.id)
...
def test_user_roles_crud(client, db_session, app):
    ...
    role = Role(role_code="ASSIGN_ROLE", role_name="Assign Role", role_type=RoleType.CUSTOM, status=RoleStatus.ACTIVE)
    db_session.add(role)
    db_session.commit()
    ...
    res = client.post(f"/users/{target_user_id}/roles", json=assign_data, headers=headers)
    assert res.status_code == 201
    
    # Verify DB state
    db_ur = db_session.query(UserRole).filter_by(user_id=target_user_id, role_id=role.id).first()
    assert db_ur is not None
    
    # 2. Duplicate Assignment should fail (Business Rule)
    res_dup = client.post(f"/users/{target_user_id}/roles", json=assign_data, headers=headers)
    ...
    assert db_session.query(UserRole).filter_by(user_id=target_user_id, role_id=role.id).first() is None
