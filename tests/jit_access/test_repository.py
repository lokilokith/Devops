...
from tests.fixtures.factories import UserFactory, RoleFactory, AccessRequestFactory, ResourceFactory
...
def test_list_grants(db_session):
    ...
    grant1 = JITAccessGrant(
        user_id=user.id, role_id=role.id, resource_id=resource.id,
        approval_request_id=ar.id, status=JITGrantStatus.ACTIVE,
        expires_at=datetime.now(timezone.utc)
    )
    grant2 = JITAccessGrant(
        user_id=user.id, role_id=role.id, resource_id=resource.id,
        approval_request_id=uuid4(), status=JITGrantStatus.PENDING,
        expires_at=datetime.now(timezone.utc)
    )
