...
from app.jit_access.models import JITAccessGrant, JITAccessSession, JITGrantStatus
from app.jit_access.service import InvalidGrantStateError, JITAccessService
...
def test_jit_session_creation():
    ...
    with (
        ...
        patch("app.jit_access.events.jit_session_created") as mock_event,
    ):
        ...
        session = service.create_session(
            grant.id, system_actor, mock_vault_service, b"dummy"
        )

        assert session.access_request_id == grant.approval_request_id
        assert session.ephemeral_secret_id == mock_secret.id
        mock_vault_service.create_secret.assert_called_once_with(
            system_actor, grant.resource_id, b"dummy"
        )
