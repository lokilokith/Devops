...
def test_permissions_repository_get_by_code_and_name_whitespace(
    permissions_repo, sample_permission
):
...
def test_permissions_repository_status_changes(permissions_repo, sample_permission):
    ...
    assert (
        permissions_repo.get_by_id(sample_permission.id).status
...
def test_permissions_repository_list_filters(permissions_repo, sample_permission):
    p2 = Permission(
        permission_code="LST2",
        permission_name="List 2",
    ...
    combo_perms = permissions_repo.list(
        status=PermissionStatus.INACTIVE, action=PermissionAction.UPDATE
    )
...
def test_permissions_repository_count_filters(permissions_repo, sample_permission):
    p2 = Permission(
        permission_code="CNT2",
        permission_name="Count 2",
        action=PermissionAction.UPDATE,
        status=PermissionStatus.INACTIVE,
    )
    permissions_repo.create(p2)
...
def test_permissions_repository_search_filters(permissions_repo, sample_permission):
    p2 = Permission(
        permission_code="SRCH2",
        permission_name="Search Two",
        action=PermissionAction.UPDATE,
        status=PermissionStatus.INACTIVE,
    )
    permissions_repo.create(p2)
    ...
    res4 = permissions_repo.search(
        permission_code="  SRCH2  ", permission_name="  Two  "
    )
    assert len(res4) == 1
...
def test_permissions_repository_sqlalchemy_errors(permissions_repo, sample_permission):
    with patch.object(
        permissions_repo._session, "add", side_effect=SQLAlchemyError("mocked error")
    ):
        with pytest.raises(PermissionsRepositoryError):
            permissions_repo.create(
                Permission(permission_code="ERR", permission_name="Error Permission")
            )

    with patch.object(
        permissions_repo._session,
        "execute",
    ...
    with patch.object(
        permissions_repo._session, "merge", side_effect=SQLAlchemyError("mocked error")
    ):
        with pytest.raises(PermissionsRepositoryError):
            permissions_repo.update(sample_permission)

    with patch.object(
        permissions_repo._session, "delete", side_effect=SQLAlchemyError("mocked error")
    ):
        with pytest.raises(PermissionsRepositoryError):
            permissions_repo.delete(sample_permission.id)

    with patch.object(
        permissions_repo._session, "commit", side_effect=SQLAlchemyError("mocked error")
    ):
        with pytest.raises(PermissionsRepositoryError):
            permissions_repo.activate(sample_permission.id)
