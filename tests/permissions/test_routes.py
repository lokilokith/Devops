...
def mock_auth(monkeypatch):
    monkeypatch.setattr("app.auth.service.AuthService.verify_token", lambda *args, **kwargs: {"sub": str(uuid4())})
    monkeypatch.setattr("app.authorization.service.AuthorizationService.has_permission", lambda *args, **kwargs: True)
...
def test_list_permissions(client, mock_auth, mock_service):
    mock_service.list_permissions.return_value = [{"id": str(uuid4()), "permission_name": "test"}]
    res = client.get("/permissions?skip=0&limit=10", headers={"Authorization": "Bearer token"})
...
def test_list_permissions_forbidden(client, monkeypatch):
    monkeypatch.setattr("app.auth.service.AuthService.verify_token", lambda *args, **kwargs: {"sub": str(uuid4())})
    monkeypatch.setattr("app.authorization.service.AuthorizationService.has_permission", lambda *args, **kwargs: False)
...
def test_create_permission(client, mock_auth, mock_service):
    mock_service.create_permission.return_value = {"id": str(uuid4()), "permission_code": "PERM1"}
    data = {
        "permission_code": "PERM1",
        "permission_name": "Permission 1"
    }
    res = client.post("/permissions", json=data, headers={"Authorization": "Bearer token"})
...
def test_update_permission(client, mock_auth, mock_service):
    uid = str(uuid4())
    mock_service.update_permission.return_value = {"id": uid, "permission_name": "Updated"}
    res = client.put(f"/permissions/{uid}", json={"permission_name": "Updated", "action": "create", "status": "active"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 200
...
def test_patch_permission(client, mock_auth, mock_service):
    uid = str(uuid4())
    mock_service.patch_permission.return_value = {"id": uid, "permission_name": "Patched"}
    res = client.patch(f"/permissions/{uid}", json={"permission_name": "Patched"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 200
...
def test_delete_permission(client, mock_auth, mock_service):
    ...
    res = client.delete(f"/permissions/{uid}", headers={"Authorization": "Bearer token"})
    assert res.status_code == 200
...
def test_invalid_uuid(client, mock_auth):
    res = client.get("/permissions/invalid-uuid", headers={"Authorization": "Bearer token"})
    assert res.status_code == 422
...
def test_validation_errors(client, mock_auth, mock_service):
    ...
    res = client.post("/permissions", json={"permission_code": "bad!", "permission_name": "Permission"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422

    # Permission name too short
    res = client.post("/permissions", json={"permission_code": "GOOD", "permission_name": "A"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422

    # Missing permission_name on update
    uid = str(uuid4())
    res = client.put(f"/permissions/{uid}", json={"status": "active", "action": "create"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422

    # Missing status on update
    res = client.put(f"/permissions/{uid}", json={"permission_name": "Name", "action": "create"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422
    
    # Missing action on update
    res = client.put(f"/permissions/{uid}", json={"permission_name": "Name", "status": "active"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422
    
    # Patch empty name
    res = client.patch(f"/permissions/{uid}", json={"permission_name": "A"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422
