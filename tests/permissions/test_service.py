...
def test_create_permission_repo_error_exists(service, mock_repo):
    data = {"permission_code": "A", "permission_name": "Permission"}
    mock_repo.exists_by_permission_code.side_effect = PermissionsRepositoryError("DB fail")
...
def test_update_permission(service, mock_repo):
    uid = uuid4()
    permission = Permission(permission_code="P", permission_name="old_n", description="old_d")
    permission.id = uid
    ...
    res = service.update_permission(uid, {
