...
def test_evaluate_retrieval_policy_requires_approval_no_request(
    engine, mock_session, mock_authz
):
    role_mock = Mock()
    role_mock.id = uuid.uuid4()
    mock_authz.get_user_roles.return_value = [role_mock]
    ...
    mock_session.execute.side_effect = [
        MagicMock(
            scalars=MagicMock(
                return_value=MagicMock(all=MagicMock(return_value=[policy_mock]))
            )
        ),
        MagicMock(
...
def test_evaluate_retrieval_policy_requires_approval_has_valid_request(
    engine, mock_session, mock_authz
):
    role_mock = Mock()
    role_mock.id = uuid.uuid4()
    ...
    mock_session.execute.side_effect = [
        ...
        MagicMock(
            scalars=MagicMock(
                return_value=MagicMock(all=MagicMock(return_value=[request_mock]))
            )
        ),
    ]
