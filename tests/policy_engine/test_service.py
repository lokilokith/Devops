...
def test_evaluation_ip_mismatch_denies(policy_service):
    ...
    policy_service._repo.create(AccessPolicy(
        name="Internal IP Only",
        conditions={"allowed_ip_ranges": ["10.0.0.0/8"]},
        effect=PolicyEffect.ALLOW
    ))
    
    res = policy_service.evaluate_policy(uuid4(), "res-123", "read", {"ip": "192.168.1.5"})
    assert res["decision"] == "DENY"
    assert "matched no ALLOW policies" in res["reason"]
    
    res2 = policy_service.evaluate_policy(uuid4(), "res-123", "read", {"ip": "10.0.0.5"})
...
def test_evaluation_time_restriction(policy_service):
    ...
    res_deny = policy_service.evaluate_policy(uuid4(), "res-123", "read", {"time": "2023-01-01T22:00:00Z"})
    assert res_deny["decision"] == "DENY"
    
    res_allow = policy_service.evaluate_policy(uuid4(), "res-123", "read", {"time": "2023-01-01T12:00:00Z"})
...
def test_evaluation_explicit_deny_wins(policy_service):
    ...
    res_allow = policy_service.evaluate_policy(uuid4(), "res-123", "read", {"ip": "10.0.0.6"})
    assert res_allow["decision"] == "ALLOW"
    
    res_deny = policy_service.evaluate_policy(uuid4(), "res-123", "read", {"ip": "10.0.0.5"})
    assert res_deny["decision"] == "DENY"
