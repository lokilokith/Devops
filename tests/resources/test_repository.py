...
def test_resources_repository_get_by_code_and_name_whitespace(
    resources_repo, sample_resource
):
...
def test_resources_repository_status_changes(resources_repo, sample_resource):
    ...
    assert (
        resources_repo.get_by_id(sample_resource.id).status == ResourceStatus.INACTIVE
    )
...
def test_resources_repository_list_filters(resources_repo, sample_resource):
    ...
    combo_res = resources_repo.list(
        status=ResourceStatus.INACTIVE, resource_type=ResourceType.DATABASE
    )
...
def test_resources_repository_sqlalchemy_errors(resources_repo, sample_resource):
    with patch.object(
        resources_repo._session, "add", side_effect=SQLAlchemyError("mocked error")
    ):
        with pytest.raises(ResourcesRepositoryError):
            resources_repo.create(
                Resource(resource_code="ERR", resource_name="Error Resource")
            )

    with patch.object(
        resources_repo._session, "execute", side_effect=SQLAlchemyError("mocked error")
    ):
        with pytest.raises(ResourcesRepositoryError):
    ...
    with patch.object(
        resources_repo._session, "merge", side_effect=SQLAlchemyError("mocked error")
    ):
        with pytest.raises(ResourcesRepositoryError):
            resources_repo.update(sample_resource)

    with patch.object(
        resources_repo._session, "delete", side_effect=SQLAlchemyError("mocked error")
    ):
        with pytest.raises(ResourcesRepositoryError):
            resources_repo.delete(sample_resource.id)

    with patch.object(
        resources_repo._session, "commit", side_effect=SQLAlchemyError("mocked error")
    ):
        with pytest.raises(ResourcesRepositoryError):
            resources_repo.activate(sample_resource.id)
