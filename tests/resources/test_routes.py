...
def test_list_resources(client, mock_auth, mock_service):
    mock_service.list_resources.return_value = [{"id": str(uuid4()), "resource_name": "test"}]
    res = client.get("/resources?skip=0&limit=10", headers={"Authorization": "Bearer token"})
...
def test_create_resource(client, mock_auth, mock_service):
    mock_service.create_resource.return_value = {"id": str(uuid4()), "resource_code": "DB1"}
    data = {
        "resource_code": "DB1",
        "resource_name": "Database 1"
    }
    res = client.post("/resources", json=data, headers={"Authorization": "Bearer token"})
...
def test_validation_errors(client, mock_auth, mock_service):
    ...
    res = client.post("/resources", json={"resource_code": "bad!", "resource_name": "Resource"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422

    # Resource name too short
    res = client.post("/resources", json={"resource_code": "GOOD", "resource_name": "A"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422

    # Missing resource_name on update
    uid = str(uuid4())
    res = client.put(f"/resources/{uid}", json={"status": "active", "resource_type": "database"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422

    # Missing status on update
    res = client.put(f"/resources/{uid}", json={"resource_name": "Name", "resource_type": "database"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422
    
    # Missing resource_type on update
    res = client.put(f"/resources/{uid}", json={"resource_name": "Name", "status": "active"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422
    
    # Patch empty name
    res = client.patch(f"/resources/{uid}", json={"resource_name": "A"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422
