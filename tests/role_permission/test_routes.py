...
def test_list_permissions_for_role(client, mock_auth, mock_service):
    role_id = str(uuid4())
    mock_service.list_permissions_for_role.return_value = [{"id": str(uuid4()), "permission_name": "test"}]
    res = client.get(f"/roles/{role_id}/permissions", headers={"Authorization": "Bearer token"})
...
def test_assign_permission(client, mock_auth, mock_service):
    ...
    res = client.post(f"/roles/{role_id}/permissions", json=data, headers={"Authorization": "Bearer token"})
...
def test_assign_permission_conflict(client, mock_auth, mock_service):
    ...
    role_id = str(uuid4())
    perm_id = str(uuid4())
    mock_service.assign_permission.side_effect = ValidationError("Permission is already assigned")
    data = {"permission_id": perm_id}
    res = client.post(f"/roles/{role_id}/permissions", json=data, headers={"Authorization": "Bearer token"})
...
def test_remove_permission(client, mock_auth, mock_service):
    ...
    res = client.delete(f"/roles/{role_id}/permissions/{perm_id}", headers={"Authorization": "Bearer token"})
    assert res.status_code == 200
...
def test_list_roles_for_permission(client, mock_auth, mock_service):
    perm_id = str(uuid4())
    mock_service.list_roles_for_permission.return_value = [{"id": str(uuid4()), "role_name": "test"}]
    res = client.get(f"/permissions/{perm_id}/roles", headers={"Authorization": "Bearer token"})
    assert res.status_code == 200
    assert res.json["success"] is True
    assert len(res.json["data"]) == 1
...
def test_invalid_uuid(client, mock_auth):
    res = client.get("/roles/invalid-uuid/permissions", headers={"Authorization": "Bearer token"})
    assert res.status_code == 422
