...
def test_assign_permission_success(service, mock_repo, mock_roles_repo, mock_permissions_repo):
    role_id = uuid4()
    permission_id = uuid4()
    user_id = uuid4()
    ...
    assert res == "rp"
    mock_repo.assign_permission_to_role.assert_called_with(role_id, permission_id, user_id)
...
def test_assign_permission_permission_not_found(service, mock_roles_repo, mock_permissions_repo):
    role_id = uuid4()
    permission_id = uuid4()
    mock_roles_repo.get_by_id.return_value = "role"
...
def test_assign_permission_permission_error(service, mock_roles_repo, mock_permissions_repo):
    role_id = uuid4()
    permission_id = uuid4()
    mock_roles_repo.get_by_id.return_value = "role"
...
def test_assign_permission_already_exists(service, mock_repo, mock_roles_repo, mock_permissions_repo):
    role_id = uuid4()
    permission_id = uuid4()
    mock_roles_repo.get_by_id.return_value = "role"
    mock_permissions_repo.get_by_id.return_value = "perm"
    mock_repo.assign_permission_to_role.side_effect = RolePermissionAlreadyExistsError("exists")
    with pytest.raises(ValidationError):
        service.assign_permission(role_id, permission_id)

def test_assign_permission_repo_error(service, mock_repo, mock_roles_repo, mock_permissions_repo):
    role_id = uuid4()
    permission_id = uuid4()
    mock_roles_repo.get_by_id.return_value = "role"
    mock_permissions_repo.get_by_id.return_value = "perm"
    mock_repo.assign_permission_to_role.side_effect = RolePermissionsRepositoryError("fail")
    with pytest.raises(RolePermissionsServiceError):
        service.assign_permission(role_id, permission_id)
...
def test_remove_permission_not_found(service, mock_repo):
    role_id = uuid4()
    permission_id = uuid4()
    mock_repo.remove_permission_from_role.side_effect = RolePermissionNotFoundError("nf")
    with pytest.raises(ValidationError):
...
def test_remove_permission_repo_error(service, mock_repo):
    role_id = uuid4()
    permission_id = uuid4()
    mock_repo.remove_permission_from_role.side_effect = RolePermissionsRepositoryError("fail")
    with pytest.raises(RolePermissionsServiceError):
        service.remove_permission(role_id, permission_id)
...
def test_list_permissions_for_role_error(service, mock_repo):
    role_id = uuid4()
    mock_repo.list_permissions_for_role.side_effect = RolePermissionsRepositoryError("fail")
    with pytest.raises(RolePermissionsServiceError):
...
def test_list_roles_for_permission_error(service, mock_repo):
    permission_id = uuid4()
    mock_repo.list_roles_for_permission.side_effect = RolePermissionsRepositoryError("fail")
    with pytest.raises(RolePermissionsServiceError):
