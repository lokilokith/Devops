...
def test_assign_permission_to_role(role_permissions_repo, sample_role, sample_permission):
    rp = role_permissions_repo.assign_permission_to_role(sample_role.id, sample_permission.id)
...
def test_assign_permission_already_exists(role_permissions_repo, sample_role, sample_permission):
    role_permissions_repo.assign_permission_to_role(sample_role.id, sample_permission.id)
    with pytest.raises(RolePermissionAlreadyExistsError):
        role_permissions_repo.assign_permission_to_role(sample_role.id, sample_permission.id)

def test_remove_permission_from_role(role_permissions_repo, sample_role, sample_permission):
    role_permissions_repo.assign_permission_to_role(sample_role.id, sample_permission.id)
    assert role_permissions_repo.exists(sample_role.id, sample_permission.id) is True
    
    role_permissions_repo.remove_permission_from_role(sample_role.id, sample_permission.id)
    assert role_permissions_repo.exists(sample_role.id, sample_permission.id) is False

def test_remove_permission_not_found(role_permissions_repo, sample_role, sample_permission):
...
def test_list_permissions_for_role(role_permissions_repo, sample_role, db_session):
    p1 = Permission(permission_code="PERM1", permission_name="Perm 1", action=PermissionAction.READ, status=PermissionStatus.ACTIVE)
    p2 = Permission(permission_code="PERM2", permission_name="Perm 2", action=PermissionAction.CREATE, status=PermissionStatus.ACTIVE)
...
def test_list_roles_for_permission(role_permissions_repo, sample_permission, db_session):
    r1 = Role(role_code="ROLE1", role_name="Role 1", role_type=RoleType.SYSTEM, status=RoleStatus.ACTIVE)
    r2 = Role(role_code="ROLE2", role_name="Role 2", role_type=RoleType.CUSTOM, status=RoleStatus.ACTIVE)
...
def test_sqlalchemy_errors(role_permissions_repo, sample_role, sample_permission):
    with patch.object(role_permissions_repo._session, 'execute', side_effect=SQLAlchemyError("mocked error")):
...
def test_assign_permission_error_after_exists(role_permissions_repo, sample_role, sample_permission):
    with patch.object(role_permissions_repo._session, 'add', side_effect=SQLAlchemyError("mocked error")):
        with pytest.raises(RolePermissionsRepositoryError):
            role_permissions_repo.assign_permission_to_role(sample_role.id, sample_permission.id)

def test_remove_permission_error_after_fetch(role_permissions_repo, sample_role, sample_permission):
    role_permissions_repo.assign_permission_to_role(sample_role.id, sample_permission.id)
    with patch.object(role_permissions_repo._session, 'delete', side_effect=SQLAlchemyError("mocked error")):
        with pytest.raises(RolePermissionsRepositoryError):
            role_permissions_repo.remove_permission_from_role(sample_role.id, sample_permission.id)
