...
def test_roles_repository_list_filters(roles_repo, sample_role):
    role2 = Role(role_code="LST2", role_name="List 2", role_type=RoleType.CUSTOM, status=RoleStatus.INACTIVE)
...
def test_roles_repository_count_filters(roles_repo, sample_role):
    role2 = Role(role_code="CNT2", role_name="Count 2", role_type=RoleType.CUSTOM, status=RoleStatus.INACTIVE)
    roles_repo.create(role2)
...
def test_roles_repository_search_filters(roles_repo, sample_role):
    role2 = Role(role_code="SRCH2", role_name="Search Two", role_type=RoleType.CUSTOM, status=RoleStatus.INACTIVE)
    roles_repo.create(role2)
...
def test_roles_repository_sqlalchemy_errors(roles_repo, sample_role):
    with patch.object(roles_repo._session, 'add', side_effect=SQLAlchemyError("mocked error")):
        with pytest.raises(RolesRepositoryError):
            roles_repo.create(Role(role_code="ERR", role_name="Error Role"))
            
    with patch.object(roles_repo._session, 'execute', side_effect=SQLAlchemyError("mocked error")):
        with pytest.raises(RolesRepositoryError):
    ...
    with patch.object(roles_repo._session, 'merge', side_effect=SQLAlchemyError("mocked error")):
        with pytest.raises(RolesRepositoryError):
            roles_repo.update(sample_role)
            
    with patch.object(roles_repo._session, 'delete', side_effect=SQLAlchemyError("mocked error")):
        with pytest.raises(RolesRepositoryError):
            roles_repo.delete(sample_role.id)
            
    with patch.object(roles_repo._session, 'commit', side_effect=SQLAlchemyError("mocked error")):
        with pytest.raises(RolesRepositoryError):
            roles_repo.activate(sample_role.id)
