...
def test_list_roles(client, mock_auth, mock_service):
    ...
    res = client.get("/roles?skip=0&limit=10", headers={"Authorization": "Bearer token"})
...
def test_update_role(client, mock_auth, mock_service):
    ...
    res = client.put(f"/roles/{uid}", json={"role_name": "Updated", "status": "active"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 200
...
def test_patch_role(client, mock_auth, mock_service):
    ...
    res = client.patch(f"/roles/{uid}", json={"role_name": "Patched"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 200
...
def test_validation_errors(client, mock_auth, mock_service):
    ...
    res = client.post("/roles", json={"role_code": "bad!", "role_name": "Role"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422

    # Role name too short
    res = client.post("/roles", json={"role_code": "GOOD", "role_name": "A"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422

    # Missing role_name on update
    uid = str(uuid4())
    res = client.put(f"/roles/{uid}", json={"status": "active"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422

    # Missing status on update
    res = client.put(f"/roles/{uid}", json={"role_name": "Name"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422
    
    # Patch empty name
    res = client.patch(f"/roles/{uid}", json={"role_name": "A"}, headers={"Authorization": "Bearer token"})
    assert res.status_code == 422
