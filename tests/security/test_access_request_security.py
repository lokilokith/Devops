...
def _grant_perm(db_session, user, perm_code, action):
    ...
    if not perm:
        perm = Permission(
            permission_code=perm_code, permission_name=perm_code, action=action
        )
        db_session.add(perm)
        db_session.flush()

    if (
        not db_session.query(RolePermission)
...
def test_assigned_approver_can_approve(
    client, db_session, normal_user, approver_user, approver_token
):
    _grant_perm(
        db_session,
        approver_user,
    ...
    # Create request
    req = AccessRequestFactory(
        requester=normal_user, status=AccessRequestStatus.PENDING
    )
    # Create workflow assigned to approver
    wf = WorkflowFactory(  # noqa: F841
        access_request_id=req.id, approver=approver_user, status=ApprovalStatus.PENDING
    )
    db_session.flush()
...
def test_user_with_approve_permission_but_not_assigned_approver(
    client, db_session, normal_user, normal_token, approver_user
):
    ...
    req = AccessRequestFactory(
        requester=approver_user, status=AccessRequestStatus.PENDING
    )
    wf = WorkflowFactory(  # noqa: F841
        access_request_id=req.id, approver=approver_user, status=ApprovalStatus.PENDING
    )
...
def test_normal_user_approval_attempt(
    client, db_session, normal_user, normal_token, approver_user
):
    req = AccessRequestFactory(
        requester=approver_user, status=AccessRequestStatus.PENDING
...
def test_requester_cancels_own_pending_request(
    client, db_session, normal_user, normal_token
):
    req = AccessRequestFactory(
        requester=normal_user, status=AccessRequestStatus.PENDING
    )
...
def test_user_attempts_cancelling_another_users_request(
    client, db_session, normal_user, normal_token, approver_user
):
...
def test_admin_cancels_another_users_request(
    client, db_session, admin_user, admin_token, normal_user
):
...
def test_approved_request_cancellation_attempt(
    client, db_session, normal_user, normal_token
):
    req = AccessRequestFactory(
        requester=normal_user, status=AccessRequestStatus.APPROVED
    )
    db_session.flush()
