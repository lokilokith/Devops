...
def test_seed_rbac_partial_state(app, db_session):
    ...
    partial_res = Resource(resource_code="RESOURCE_USERS", resource_name="User Management")
    ...
    soc_roles = db_session.scalars(select(Role).where(Role.role_code == "SOC_ANALYST")).all()
    assert len(soc_roles) == 1
    
    # Ensure there are no duplicate RESOURCE_USERS resources
    users_res = db_session.scalars(select(Resource).where(Resource.resource_code == "RESOURCE_USERS")).all()
