...
def test_approve_workflow_success(service):
    ...
    wf = ApprovalWorkflow(id=wf_id, status=ApprovalStatus.PENDING, approver_id=approver_id, access_request_id=req_id)
    service._repo.get_by_id.return_value = wf
    service._ar_repo.get_by_id.return_value = Mock(id=req_id, requested_role_id=uuid4(), requester_id=uuid4())
...
def test_reject_workflow_success(service):
    ...
    wf = ApprovalWorkflow(id=wf_id, status=ApprovalStatus.PENDING, approver_id=rejecter_id, access_request_id=req_id)
    service._repo.get_by_id.return_value = wf
    service._ar_repo.get_by_id.return_value = Mock(id=req_id, requested_role_id=uuid4(), requester_id=uuid4())
...
def test_invalid_state_transition(service):
    ...
    wf = ApprovalWorkflow(id=wf_id, status=ApprovalStatus.APPROVED, approver_id=approver_id)
    service._repo.get_by_id.return_value = wf
