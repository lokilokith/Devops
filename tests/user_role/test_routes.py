...
def test_list_roles_for_user(client, mock_auth, mock_service):
    user_id = str(uuid4())
    mock_service.list_roles_for_user.return_value = [{"id": str(uuid4()), "role_name": "test"}]
    res = client.get(f"/users/{user_id}/roles", headers={"Authorization": "Bearer token"})
...
def test_assign_role(client, mock_auth, mock_service):
    ...
    res = client.post(f"/users/{user_id}/roles", json=data, headers={"Authorization": "Bearer token"})
...
def test_remove_role(client, mock_auth, mock_service):
    ...
    res = client.delete(f"/users/{user_id}/roles/{role_id}", headers={"Authorization": "Bearer token"})
    assert res.status_code == 200
...
def test_list_users_for_role(client, mock_auth, mock_service):
    role_id = str(uuid4())
    mock_service.list_users_for_role.return_value = [{"id": str(uuid4()), "username": "test"}]
    res = client.get(f"/roles/{role_id}/users", headers={"Authorization": "Bearer token"})
    assert res.status_code == 200
    assert res.json["success"] is True
    assert len(res.json["data"]) == 1
...
def test_invalid_uuid(client, mock_auth):
    res = client.get("/users/invalid-uuid/roles", headers={"Authorization": "Bearer token"})
    assert res.status_code == 422
