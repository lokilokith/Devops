...
def test_create_secret_success(service, mock_authz, mock_crypto, mock_repo, mock_audit):
    ...
    with patch(
        "app.vault.service.SecretFactory.create_new_secret", return_value=mock_secret
    ) as mock_factory:
        mock_crypto.encrypt_payload.return_value = (
            b"enc_dek",
            b"enc_payload",
            Mock(spec=SecretMetadata),
        )
...
def test_retrieve_secret_success_allow(
    service, mock_authz, mock_policy, mock_repo, mock_crypto, mock_audit
):
    actor_id = uuid.uuid4()
...
def test_retrieve_secret_require_approval(
    service, mock_authz, mock_policy, mock_repo, mock_audit
):
    actor_id = uuid.uuid4()
    secret_id = uuid.uuid4()
...
def test_retrieve_secret_denied_by_policy(
    service, mock_authz, mock_policy, mock_repo, mock_audit
):
    secret_id = uuid.uuid4()
    actor_id = uuid.uuid4()
...
def test_rotate_secret_success(service, mock_authz, mock_crypto, mock_repo, mock_audit):
    actor_id = uuid.uuid4()
    secret_id = uuid.uuid4()
    new_plaintext = b"new-secret"
    ...
    result = service.rotate_secret(actor_id, secret_id, new_plaintext)
...
def test_disable_secret_success(service, mock_authz, mock_repo, mock_audit):
    actor_id = uuid.uuid4()
    secret_id = uuid.uuid4()
    ...
    result = service.disable_secret(actor_id, secret_id)
    assert result == mock_secret
    mock_secret.disable.assert_called_once()
    mock_repo.save.assert_called_once_with(mock_secret)
    mock_audit.log_event.assert_called_once()


def test_list_secrets(service, mock_authz, mock_repo):
    ...
    type(mock_secret).resource_id = PropertyMock(return_value=uuid.uuid4())
    type(mock_secret).status = PropertyMock(return_value=SecretStatus.ACTIVE)
    type(mock_secret).created_at = PropertyMock(return_value=datetime.now(timezone.utc))

    mock_repo.list_active_secrets.return_value = [mock_secret]

    result = service.list_secrets(actor_id)
    assert len(result) == 1
    mock_authz.authorize.assert_called_once()


def test_get_statistics(service, mock_authz, mock_repo):
    actor_id = uuid.uuid4()
    mock_repo.count_all.return_value = 10
    mock_repo.count_by_status.return_value = 5

    stats = service.get_statistics(actor_id)
