...
from app.resources.models import Resource, ResourceType, ResourceStatus, Environment, Criticality


def test_vault_rotation_updates_lifecycle_policy(db_session, client, admin_token):
    # 1. Create a Resource
    res = Resource(id=uuid.uuid4(), resource_name="Test Res E2E", resource_code="test-res-e2e", resource_type=ResourceType.SERVER, status=ResourceStatus.ACTIVE, environment=Environment.DEV, criticality=Criticality.LOW)
    db_session.add(res)
    ...
    assert (updated_policy.next_rotation_at - updated_policy.last_rotated_at).total_seconds() == 3600
