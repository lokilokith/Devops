...
def test_create_and_get_policy(db_session):
    ...
    res = Resource(id=uuid.uuid4(), resource_name="Test Res", resource_code="test-res", resource_type=ResourceType.SERVER, status=ResourceStatus.ACTIVE, environment=Environment.DEV, criticality=Criticality.LOW)
    db_session.add(res)
    
    secret = VaultSecret(id=uuid.uuid4(), resource_id=res.id, status=SecretStatus.ACTIVE, row_version=1)
