...
def test_expired_active_lease_is_processed(db_session, checkout_svc, test_user):
    ...
    fresh_lease = db_session.execute(
        select(CredentialLease).where(CredentialLease.id == lease.id)
    ).scalar()
...
def test_non_expired_lease_untouched(db_session, checkout_svc, test_user):
    ...
    ar_id = _make_ar(db_session, test_user.id, res.id)
    lease = _make_lease(  # noqa: F841
        db_session, sec.id, test_user.id, ar_id, expired=False
    )
...
def test_secret_state_changes_correctly_after_expiration(
    db_session, checkout_svc, test_user
):
    res = _make_resource(db_session)
    sec = _make_secret(db_session, res)
    _make_policy(db_session, sec.id)
...
def test_multiple_leases_process_independently(db_session, checkout_svc, test_user):
    ...
    db_session.expire_all()
    fresh_lease1 = db_session.execute(
        select(CredentialLease).where(CredentialLease.id == lease1.id)
    ).scalar()
    fresh_lease2 = db_session.execute(
        select(CredentialLease).where(CredentialLease.id == lease2.id)
    ).scalar()
