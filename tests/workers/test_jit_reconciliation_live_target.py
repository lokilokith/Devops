...
from app.execution.domain import ExecutionRequest, ExecutionOperation, ExecutionAuthorizationContext, ExecutionStatus
from app.execution.ssh_executor import SSHTargetExecutor, SSHExecutionConfig, SSHConnectionContext
...
from app.resources.models import Resource, ResourceType, ResourceStatus, Environment
...
def live_reconciliation_worker(db_session, live_executor, svc_key):
    ...
    return JITReconciliationWorker(jit_repo, audit_svc, live_executor, target_repo, bootstrap_credential=svc_key)
...
def provision_and_apply(live_executor, cfg, pub_key_str):
    ...
    assert res_prov.status == ExecutionStatus.SUCCESS, f"Provision failed: {res_prov.error_message} {res_prov.details}"
    ...
    assert res_apply.status == ExecutionStatus.SUCCESS, f"Apply failed: {res_apply.error_message} {res_apply.details}"
...
def test_scenario_a_active_missing_expected_privilege(configured_live_grant, live_executor, live_reconciliation_worker, db_session):
...
def test_scenario_b_already_authorized_deterministic_cleanup(configured_live_grant, live_executor, live_reconciliation_worker, db_session):
    cfg = configured_live_grant
    priv, pub = generate_ed25519_keypair()
    provision_and_apply(live_executor, cfg, pub)
...
def test_scenario_c_security_uncertain_recovery(configured_live_grant, live_executor, live_reconciliation_worker, db_session):
    # C1 Clean Target
    cfg = configured_live_grant
...
def test_scenario_d_unrelated_artifact(configured_live_grant, live_executor, live_reconciliation_worker, db_session):
    cfg = configured_live_grant
    priv, pub = generate_ed25519_keypair()
    provision_and_apply(live_executor, cfg, pub)
...
def test_scenario_e_unmanaged_process(configured_live_grant, live_executor, live_reconciliation_worker, db_session):
    cfg = configured_live_grant
    priv, pub = generate_ed25519_keypair()
    provision_and_apply(live_executor, cfg, pub)
...
def test_scenario_f_fail_closed(configured_live_grant, live_executor, live_reconciliation_worker, db_session):
    cfg = configured_live_grant
