...
from app.jit_access.models import JITGrantStatus, JITAccessGrant, ReconciliationStatus, JITReconciliationState
from app.execution.domain import ExecutionStatus, ExecutionResult
...
def worker(mock_session, mock_repo, mock_executor):
    ...
    w = JITReconciliationWorker(repository=mock_repo_wrapper, audit_service=Mock(), executor=mock_executor, target_account_repo=mock_repo, bootstrap_credential=b"test-bootstrap")
    return w
...
def test_safe_retry_execution(worker, mock_session, mock_repo, mock_executor):
    grant = JITAccessGrant(
        ...
        target_account_binding_id=uuid.uuid4(), resource_id=uuid.uuid4(), user_id=uuid.uuid4()
    )
    binding = TargetAccountBinding(id=grant.target_account_binding_id, target_os_username="root")
    
    # DB lookups
    mock_session.get.return_value = grant
    mock_session.execute.return_value.scalar_one_or_none.return_value = JITReconciliationState(worker_id=worker.worker_id)
    ...
    mock_executor.inspect_target_state.return_value = ExecutionResult(
        execution_id=uuid.uuid4(), operation="test", verification_status=None,
        status=ExecutionStatus.SUCCESS,
        details={"sudoers_present": True, "active_sessions": []}
    )
    
    # Mock remediation (terminate + revoke)
    mock_executor.terminate_jit_sessions.return_value = ExecutionResult(execution_id=uuid.uuid4(), operation="test", verification_status=None, status=ExecutionStatus.SUCCESS)
    mock_executor.revoke_jit_grant.return_value = ExecutionResult(execution_id=uuid.uuid4(), operation="test", verification_status=None, status=ExecutionStatus.SUCCESS)
...
def test_safe_retry_execution_failure(worker, mock_session, mock_repo, mock_executor):
    ...
    mock_executor.terminate_jit_sessions.return_value = ExecutionResult(execution_id=uuid.uuid4(), operation="test", verification_status=None, status=ExecutionStatus.SUCCESS)
    mock_executor.revoke_jit_grant.return_value = ExecutionResult(execution_id=uuid.uuid4(), operation="test", verification_status=None, status=ExecutionStatus.FAILED, error_message="SSH Fail")
...
def test_security_uncertain_recovery(worker, mock_session, mock_repo, mock_executor):
    grant = JITAccessGrant(
        ...
        resource_id=uuid.uuid4(), user_id=uuid.uuid4(),
        row_version=1
    )
    binding = TargetAccountBinding(id=grant.target_account_binding_id, target_os_username="root")
    ...
    assert result["reason"] == "Uncertain grant proved cleanly removed (Recovered from SECURITY_UNCERTAIN to REVOKED)"
